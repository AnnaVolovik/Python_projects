...
@app.route('/project_two', methods=['GET'])
@cross_origin()
def project_two():
    ...
    page_limit = 1  # by default for demonstration purposes we only go through first two pages
    ...
    # backup - get the same information with requests if failed do so with selenium
    if not res:
...
def export_results_to_db(res):
    ...
    try:
        ...
        db.session.rollback()
        return json.dumps({'error': f"Data Base Error. Please try init_db"})
